...
class qarm:
    ...
    def control_gripper(self, deg):
        if abs(deg) <= 45 and math.degrees(self.g + math.radians(deg)) >= 0 and math.degrees(
                self.g + math.radians(deg)) < 46:
            self.g = self.g + math.radians(deg)
            self.my_qarm.qarm_move_gripper(self.g, wait = False)
        else:
            print("Invalid Angle. Gripper rotate between 0 and 45 degrees.")
    ...
    def move_arm(self, x, y, z):
        ...
        base, shoulder, elbow = self.my_qarm.qarm_inverse_kinematics(x, y, z)
    ...
    def move_arm_intermediate(self):
        base, shoulder, elbow = self.my_qarm.qarm_inverse_kinematics(0.406, 0, 0.483) # First move home without affecting the wrist or gripper
        # print(math.degrees(base))
        # print(math.degrees(shoulder))
        # print(math.degrees(elbow))
